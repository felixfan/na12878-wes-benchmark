#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fig1_design.py — Figure 1: study-design schematic. This is the only figure that is a drawing
rather than a data plot, so it has no R twin and no source-data file.
One NA12878 DNA aliquot → three platforms → depth titration × caller × hardware × reference build
→ GIAB benchmark → Precision/Recall/F1. Writes figures/Fig1_design.png (300 dpi).

    python figure_code/Fig1_design.py
"""
import os
import matplotlib
matplotlib.use("Agg")
...
def main():
    ...
    root = os.path.dirname(here) if os.path.isdir(os.path.join(here, "..", "source_data")) else here
    outdir = os.path.join(root, "figures")
    os.makedirs(outdir, exist_ok=True)
    out = os.path.join(outdir, "Fig1_design.png")
    ...
    box(ax, 5, 8.5, 8.6, 1.1,
        ...
        "5·8·10·12·15·20·25·30·40·50·60·75·100·150·200·300·400× and full  (seeds 42/43/44)",
